...
alert("JavaScipt loaded")
var theQuizMapp = [chart, nodeStructure, Q1];

alert("tree loaded")

//testing the answer creator
appendFourAnswers(theQuizMapp, Q1);
appendLinkNode(theQuizMapp, Q1);
appendLinkNode(theQuizMapp, nodeStructure);



alert("append success")
myTree = new Treant(theQuizMapp, null, $);
...
{
  for(index = 1; index < theQuizMapp.length; index++)
  {
    if(theQuizMapp[index].text.title == givenTitle)
      return theQuizMapp[index];
  } // for loop
...
function addLButtonAction(givenButton)
{
  ...
  //to the link Table
  appendQuestion(theQuizMapp, childNode);
...
function appendLinkNode(givenTreeArray, parentNode)
{
  ...
  givenTreeArray.push(new LinkNode(parentNode));
